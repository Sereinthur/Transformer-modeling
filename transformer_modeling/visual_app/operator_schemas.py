...
OPERATOR_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "minimax_sparse_attention": {
        "slot": "attention",
        "chinese_name": "MiniMax Sparse Attention (MSA)",
        "implementations": ["default", "block_sparse"],
        "params": {
            "query_heads": {"type": "int", "min": 1, "default": 64, "label": "Query头数", "required": True},
            "kv_heads": {"type": "int", "min": 1, "default": 4, "label": "KV头数", "required": True,
                         "hint": "GQA：query_heads必须能被KV头数整除"},
            "head_dim": {"type": "int", "min": 1, "default": 128, "label": "单头维度", "required": True},
            "indexer_heads": {"type": "int", "min": 1, "default": 4, "label": "Indexer Query头数", "required": True},
            "indexer_head_dim": {"type": "int", "min": 1, "default": 128, "label": "Indexer头维度", "required": True},
            "block_size": {"type": "int", "min": 1, "default": 128, "label": "KV block大小", "required": True},
            "topk_blocks": {"type": "int", "min": 1, "default": 16, "label": "选中block数", "required": True,
                            "hint": "当前local block包含在该Top-K预算内"},
            "rope_dim": {"type": "int", "min": 0, "default": 64, "label": "Partial RoPE维度", "required": False},
            "kv_cache_dtype": _kv_cache_dtype(),
            "weight_dtype": _weight_dtype(),
        },
        "description": "MiniMax M3的MSA：Indexer扫描独立Index-K cache，按128-token block max score选择Top-K块，再对命中块执行精确GQA Softmax Attention。",
    },
    "kda": {
        "slot": "attention",
