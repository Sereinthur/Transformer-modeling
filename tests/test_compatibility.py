...
class PublicInterfaceTests(unittest.TestCase):
    def test_top_level_api_is_stable_and_v2_only(self):
        self.assertIs(Config, DomainConfig)
        self.assertTrue(callable(estimate))
        self.assertEqual(estimate(Config.from_dict(example()), False)["schema_version"], 2)
    ...
    def test_new_public_catalog_and_resolver(self):
        ...
        resolved = resolve_model_definition(preset_id="qwen3-0.6b")
        self.assertEqual(resolved["resolved_model"]["layer_pattern"][0]["attention"]["type"], "standard_attention")

    def test_huggingface_config_maps_to_slots(self):
        ...
        self.assertEqual(resolved["layer_pattern"][0]["ffn"]["type"], "gated_ffn")
